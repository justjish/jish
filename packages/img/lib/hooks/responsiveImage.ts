...
const sizeComparator = (resp1: ResponsiveSize, resp2: ResponsiveSize): number => {
  return ((resp1?.minWidth ?? -Infinity) - (resp2?.minWidth ?? -Infinity)) - ((resp1?.maxWidth ?? Infinity) - (resp2?.maxWidth || Infinity));
};

const sizeConverter = (resp: ResponsiveSize): string =>
 typeof resp?.minWidth === 'number' && typeof resp?.maxWidth === 'number'
    ? `(min-width: ${resp.minWidth}px) and (max-width: ${resp.maxWidth}px) ${resp.size.width}px` 
    : typeof resp?.minWidth === 'number'
    ? `(min-width: ${resp.minWidth}px) ${resp.size.width}px`
    : typeof resp?.maxWidth === 'number'
...
export function useResponsiveImage(
  ...
  for (const multiplier of multipliers) {
    for (const { size } of responsive) {
      const srcSetUrl = loader(image.src || "", loaderUrl, {
        width:
          typeof size.width === "number" ? size.width * multiplier : size.width,
        height:
  ...
  if (responsive.length === 1 && (responsive[0]?.maxWidth != null || responsive[0]?.maxWidth != undefined)) {
